...
public class hello {
    ...
    public static void st_creator() {                                       //Creating log file
        try {
            if (st.exists()==false) {
                ...
                try {
                    PrintWriter snaping = new PrintWriter(new FileWriter(st, false));           //To overwrite any file with esame existing name
                    snaping.write(" ");
                    snaping.close();
                }
    ...
    public static void snapshot() {                                         //Snapshoting
        System.out.println("\nSnapshot");
        try {
            ...
            for (int i = 0; i < stop_list.length; i++) {
                snaping.append("\n\n * * * * * * * * * * * * * * * * * * * * Stop: "+(i+1)+" * * * * * * * * * * * * * * * * * * * * ");
                snaping.append("\nMinimum Waiting People: "+stop_list[i].minNum);
                snaping.append("\nMaximum Waiting People: "+stop_list[i].maxNum);
                snaping.append("\nTotal people in Queue: "+stop_list[i].totalPeople);
                snaping.append("\nTotal buses that arrived: "+stop_list[i].totalBuses);
                snaping.append("\nPeople still in Queue: "+stop_list[i].people);

                if (stop_list[i].totalBuses == 0) {
                    stop_list[i].totalBuses = 1;
                }
                snaping.append("\nAverage Waiting People: "+((stop_list[i].totalPeople)/stop_list[i].totalBuses));

                stop_list[i].minNum = -1;
                stop_list[i].maxNum = -1;
                stop_list[i].totalPeople = 0;
                stop_list[i].totalBuses = 0;
    ...
    public static void initialize() {                                       //Initializing
        ...
        for (int i = 0; i < initial_values.simulation_time/3600; i++) {
    ...
    public static void event_handler(event e) {
        if(e.type == 1) {                                                          //Arrival of Person Handler
            ...
            stop_list[e.stop-1].totalPeople = stop_list[e.stop-1].totalPeople + 1;      //Increasing total number of people for that hour

            if (stop_list[e.stop-1].minNum == -1) {                                     //To initialize
                stop_list[e.stop-1].minNum = stop_list[e.stop-1].people;
            }

            if (stop_list[e.stop-1].maxNum == -1) {                                     //To initialize
                stop_list[e.stop-1].maxNum = stop_list[e.stop-1].people;
            }

            if (stop_list[e.stop-1].people < stop_list[e.stop-1].minNum) {                      //updating minNum
                stop_list[e.stop-1].minNum = stop_list[e.stop-1].people;
            }

            if (stop_list[e.stop-1].people > stop_list[e.stop-1].maxNum) {                  //updating maxNum
                stop_list[e.stop-1].maxNum = stop_list[e.stop-1].people;
            }

            event e1 = new event(systemClock + getNextPersonTime(), 1, 0, e.stop);              //Generate event for next persons arrival
            addEvent(e1);
        ...
        if(e.type == 2) {                                                           //Arrival of Bus handler
            if(stop_list[e.stop-1].bus_at_stop == 0) {                              //No bus at the stop so enter
                ...
                stop_list[e.stop-1].totalBuses = stop_list[e.stop-1].totalBuses + 1;                //Increasing the number of buses at the stop

                stop_list[e.stop-1].bus_at_stop = 1;                            //Setting to indicate bus at stop
                ...
                if (stop_list[e.stop-1].people == 0) {                                                  //If the queue at stop is empty
                    ...
                    stop_list[tempStop-1].bus_at_stop=0;                            //To indicate no bus at the stop

                    bus_list[e.bus - 1].pos = 2;                                                    //To indicate the bus is in motion
        ...
        if (e.type == 3) {                                                          //Boarder Event Handler
            ...
            if (stop_list[e.stop-1].people == 0) {                                              //If queue becomes empty
                ...
                bus_list[e.bus-1].pos = 2;                                          //To indicate bus is in motion

                stop_list[tempStop-1].bus_at_stop=0;                                //To indicate no bus is at stop
                if (stop_list[tempStop-1].busesWaiting.size()>0){                                             //Check if buses are waiting in queue at stop
                    int nextBus = stop_list[tempStop-1].busesWaiting.poll().name;                             //Find the name of bus next in line
                    event e2 = new event(systemClock, 2, nextBus, tempStop);                                  //Generate event for bus to arrive at stop
                    addEvent(e2);
    ...
    public static double getNextPersonTime() {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            snaping.append("Average waiting people = (Total people coming into queue)/(Total buses arrived at the stop)");
            snaping.close();
        }
        ...
        while(systemClock<initial_values.simulation_time) {
            event e1 = elq.poll();
            systemClock = e1.time;
